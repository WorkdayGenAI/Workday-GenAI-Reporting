...
\begin{tikzpicture}[
  ...
  layerlabel/.style={
    font=\footnotesize\sffamily\bfseries, text=black!50, rotate=90
  }
]
...
% ── Presentation Layer ──
\node[box, fill=uilayer!15, below=of user] (spa) {SPA Dashboard};
\node[smallbox, fill=uilayer!10, right=2.5cm of spa] (discovery_ui) {Discovery UI};

% ── API Layer ──
\node[box, fill=apilayer!20, below=1.5cm of spa] (api) {FastAPI REST + SSE};
\node[smallbox, fill=apilayer!15, right=2.5cm of api] (disc_api) {Discovery API};

% ── Orchestration Layer ──
\node[box, fill=logiclayer!20, below=1.5cm of api] (orchestrator) {Workflow Orchestrator};

% ── Agent Layer ──
\node[smallbox, fill=logiclayer!15, below left=1.5cm and 0.5cm of orchestrator] (migration) {Report Config\\Package Agent};
\node[smallbox, fill=logiclayer!15, below=1.5cm of orchestrator] (dashboard) {Dashboard Config\\Package Agent};
\node[smallbox, fill=logiclayer!15, below right=1.5cm and 0.5cm of orchestrator] (export) {Export Definitions\\Agent};

% ── Runner Engine ──
\node[box, fill=storagelayer!15, below=1.2cm of dashboard] (runner) {Config-Driven Runner};

% ── AI Layer ──
\node[box, fill=ailayer!20, right=2.5cm of orchestrator] (ai_pipeline) {AI Discovery Pipeline};
\node[smallbox, fill=ailayer!15, below left=1.2cm and -0.5cm of ai_pipeline] (bm25) {BM25 Engine};
\node[smallbox, fill=ailayer!15, below right=1.2cm and -0.5cm of ai_pipeline] (llm) {LLM Scorer};

% ── External ──
\node[box, fill=storagelayer!20, below=1.2cm of runner] (playwright) {Playwright Browser};
\node[smallbox, fill=infralayer!15, right=2.5cm of llm] (groq) {Groq Cloud API};
\node[smallbox, fill=infralayer!15, below=1cm of playwright] (workday) {Workday DPT3\\Tenant};

% ── Arrows ──
\draw[arrow] (user) -- (spa);
\draw[arrow] (user.east) -| (discovery_ui);
\draw[arrow] (spa) -- (api);
...
\draw[arrow] (migration.south) -- ++(0,-0.4) -| (runner);
\draw[arrow] (dashboard) -- (runner);
\draw[arrow] (export.south) -- ++(0,-0.4) -| (runner);
...
% ── SSE Arrow ──
\draw[darrow, dashed, draw=apilayer] (api.west) -- ++(-1.5,0) |- (spa.west) node[midway, left, font=\tiny\sffamily, text=apilayer] {SSE};
...
|   |-- data/
|   |   |-- All_Custom_Reports_with_Reference_ID.json
|   |-- prompts/
...
git clone https://github.com/rishabhsaklani08/
    Reporting_Orchestrator_Agent.git
